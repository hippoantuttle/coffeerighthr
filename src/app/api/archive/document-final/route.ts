  ...
  try {
    ...
    const [
      ...
      s
        .from("recruitments")
        .select(
          "id,name,cohort,current_stage,minimum_document_reviews,created_at",
        )
        .eq("id", recruitmentId)
        .single(),
      s
      ...
      s
        ...
        .select("id,stage,title,description,weight,sort_order,is_active")
        .eq("recruitment_id", recruitmentId)
        .order("stage")
        .order("sort_order"),
    ]);
    ...
    const [
      ...
      applicantIds.length
        ? s
            ...
            .select(
              "id,applicant_id,question_key,question_label,answer,sort_order",
            )
      ...
      applicantIds.length
        ? s
            ...
            .select(
              "id,applicant_id,reviewer_id,reviewer_name,comment,status,submitted_at,created_at,updated_at",
            )
            .in("applicant_id", applicantIds)
        : Promise.resolve({ data: [], error: null }),
    ]);
    ...
    const applicantCodeById = new Map(
      (applicants ?? []).map((a) => [a.id, a.applicant_code]),
    );
    const reviewApplicantById = new Map(
      (reviews ?? []).map((r) => [r.id, r.applicant_id]),
    );
    const criterionTitleById = new Map(
    ...
    zip.file(
      "applicants.csv",
      "\ufeff" + toCsv(applicantHeaders, applicantRows),
    );
    zip.file(
      ...
      "\ufeff" +
        toCsv(
          ...
          (answers ?? []).map((a) => ({
            ...a,
            applicant_code: applicantCodeById.get(a.applicant_id),
          })),
        ),
    );
    zip.file(
      ...
      "\ufeff" +
        toCsv(
          ...
          (reviews ?? []).map((r) => ({
            ...r,
            applicant_code: applicantCodeById.get(r.applicant_id),
          })),
        ),
    );
    zip.file(
          ...
          (scores ?? []).map((sc) => {
            ...
            return {
              applicant_code: applicantCodeById.get(
                reviewApplicantById.get(sc.review_id),
              ),
              reviewer_name: review?.reviewer_name ?? "",
              criterion:
                criterionTitleById.get(sc.criterion_id) ?? sc.criterion_id,
              score: sc.score,
            };
          }),
        ),
    ...
    const manifest = {
      ...
      submitted_review_count: (reviews ?? []).filter(
    ...
    const safeCohort = String(recruitment?.cohort ?? "recruitment").replace(
      /[^0-9A-Za-z가-힣_-]/g,
      "_",
    );
    ...
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "아카이브 생성 실패" },
      { status: 500 },
    );
  }
}
