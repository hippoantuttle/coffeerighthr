...
export async function GET(req: Request) {
  try {
    ...
    const [
      ...
      s
        .from("recruitments")
        .select("minimum_interview_reviews,final_target_count")
        .eq("id", recruitmentId)
        .single(),
      s
        .from("applicants")
        .select(
          "id,applicant_code,name,major,interview_availability,final_status,final_version",
        )
    ...
    const [
      ...
      ids.length
        ? s
            ...
            .select(
              "applicant_id,scheduled_at,duration_minutes,interviewer_names,room,mode",
            )
            .in("applicant_id", ids)
    ...
    const rows = (applicants ?? []).map((applicant) => {
      const appReviews = submitted.filter(
        (r) => r.applicant_id === applicant.id,
      );
      const appReviewIds = new Set(appReviews.map((r) => r.id));
      const aggregate = interviewAggregate(
        (scores ?? []).filter((row) => appReviewIds.has(row.review_id)),
        (criteria ?? []).map((c) => ({ id: c.id, weight: Number(c.weight) })),
      );
      return {
        ...applicant,
        assignment:
          (assignments ?? []).find((a) => a.applicant_id === applicant.id) ??
          null,
        hermes_ready: (artifacts ?? []).some(
          (a) => a.applicant_id === applicant.id,
        ),
    ...
    return NextResponse.json({
      rows,
      minimumInterviewReviews: minimum,
      finalTargetCount: recruitment?.final_target_count ?? null,
    });
  } catch (error) {
    return apiError(error, "면접 현황 조회 실패");
  }
}
